...
def add_f(): # fazer uma função aq
    ...
    if tipo == 1:
        ...
        add = C("Empresa/json/Gerente_db.json")
        add.open_json()
        ...
        add = C("Empresa/json/Estagiario_db.json")
        add.open_json()
        add.add_f(new_f.to_dict())
    else:
...
def del_f():
    ...
    if tipo == 1:  
        ...
        delete_f = C("Empresa/json/Gerente_db.json")
        delete_f.open_json()
        delete_f.remove_f(f)
    elif tipo == 2:  
        f = input('Informe o nome do Estagiarios para exclusão:\n nome = ').lower()
        delete_f = C("Empresa/json/Estagiario_db.json")
        delete_f.open_json()
        delete_f.remove_f(f)
